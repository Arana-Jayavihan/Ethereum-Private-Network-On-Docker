...
currentDir=$(pwd)
cp templates/envTemplate .env
sed -i "s/"CHAINID"/$CHAINID/" .env
sed -i "s/"NODE1_ACC_PUB"/0x$pubKey1/" .env
sed -i "s/"NODE2_ACC_PUB"/0x$pubKey2/" .env
sed -i "s/"NODE3_ACC_PUB"/0x$pubKey3/" .env
sed -i "s/"NODE4_ACC_PUB"/0x$pubKey4/" .env
sed -i "s/"BOOTNODE_PUB_KEY"/$bootNodeEnode/" .env
sed -i "s@"CURRENT_WORK_DIR"@$currentDir@" .env
